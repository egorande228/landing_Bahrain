...
.bahrain-logo {
  position: relative;
  display: inline-flex;
  width: min(100%, var(--bahrain-logo-frame-width, 186px));
  max-width: 100%;
  align-items: center;
  justify-content: center;
  min-width: 0;
  padding: var(--bahrain-logo-pad-y, 9px) var(--bahrain-logo-pad-x, 12px);
  perspective: 1100px;
  transform-style: preserve-3d;
}

.bahrain-logo__wordmark {
  position: relative;
}

.bahrain-logo__svg {
  display: block;
  width: min(100%, var(--bahrain-logo-width, 156px));
  height: auto;
  overflow: visible;
  filter: drop-shadow(0 10px 20px rgba(0, 0, 0, 0.2));
  animation: bahrainLogoFloat 7s ease-in-out infinite;
}

.bahrain-logo__letter {
...
.bahrain-logo__letter path {
  fill: currentColor;
}

.bahrain-logo__letter--white {
  color: #f6ead0;
}

.bahrain-logo__letter--gold {
  color: #FFC100;
}

.bahrain-logo--flip .bahrain-logo__letter {
  animation: bahrainLogoLetterFlip 4.2s cubic-bezier(0.22, 1, 0.36, 1) infinite;
  animation-delay: var(--letter-delay, 0ms);
}

@keyframes bahrainLogoFloat {
  0%,
  100% {
...
@keyframes bahrainLogoLetterFlip {
  0%,
  12%,
  100% {
...
export default function BahrainLogo({
  ariaLabel = "MELBET Bahrain",
  decorative = false,
  size = "md",
}: {
  ariaLabel?: string;
  ...
  return (
    <span
      className="bahrain-logo bahrain-logo--flip"
      style={
        {
          ["--bahrain-logo-width" as string]: sizes[size].width,
          ["--bahrain-logo-frame-width" as string]: sizes[size].frameWidth,
          ["--bahrain-logo-pad-x" as string]: sizes[size].padX,
          ["--bahrain-logo-pad-y" as string]: sizes[size].padY,
        } as CSSProperties
      }
    >
      <span className="bahrain-logo__wordmark">
        <svg
          aria-hidden={decorative}
          aria-label={decorative ? undefined : ariaLabel}
          className="bahrain-logo__svg"
          focusable="false"
        ...
        >
          {LETTERS.map((letter, index) => (
            <g
              key={letter.key}
              className={`bahrain-logo__letter bahrain-logo__letter--${letter.tone}`}
